...
int	execute_command(t_env *lst_env, char *cmd, char **builtins)
{
	char	cwd[2048];
	...
	{
		imprimir_tabla(var_to_array(lst_env->lst));
	}
	else if (encuentra_export(cmd) == 0)
	{
		add_node_before_last(lst_env->lst, cmd);
	}
	else if (encuentra_echo(cmd) == 0)
	{
		print_echo(cmd, lst_env->lst);
	}
	return (0);
}
...
int	work_to_list(t_env *lst_env, char *cmd)
{
	char	**builtins;
	...
	builtins[5] = "env";
	builtins[6] = "exit";
	execute_command(lst_env, cmd, builtins);
	while (lst_env->lst && lst_env->lst->prev != NULL)
		lst_env->lst = lst_env->lst->prev;
	free(builtins);
	return (0);
}
